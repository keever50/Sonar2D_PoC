...
{
  static int p;
  sample->L = (uint32_t)(wav_pcm2.data[p]-0x7F)<<24;
  sample->R = (uint32_t)(wav_pcm2.data[p]-0x7F)<<24;
...
/*TODO CHECK STEREO*/
    pwm.write((uint16_t)(sample.L>>16), true);
    pwm.write((uint16_t)(sample.R>>16), true);
  }
...
void setup() {
  ...
  if(!file0)
  {
    ...
    if(!wav_load_pcm(file0, &wav_pcm0))
    {
      ...
      audioGenerator0.setVolume(0.35f);
  ...
  if(!file1)
  {
    ...
    if(!wav_load_pcm(file1, &wav_pcm1))
    {
      ...
      audioGenerator1.setVolume(0.2f);
  ...
  if(!file2)
  {
    ...
    if(!wav_load_pcm(file2, &wav_pcm2))
    {
      ...
      audioGenerator2.setVolume(0.8f);
      mixer_output.set_channel(2, &audioGenerator2);
    }
  }
  file2.close();

  pwm.setBuffers(4, 32*2); // Give larger buffers since we're are 48khz sample rate
